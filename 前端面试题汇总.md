...
1. **hash模式和history模式有什么区别？**
    hash模式的路径永远带着#号，默认开发都使用这个模式。原理是通过监听`window.onhashchange`事件，做相应的前端响应。
    history模式更像普通正常的path，更有利于进行分享和推广，而且有些APP分享链接不允许带#号，这时更适合使用history模式。history模式通过HTML5的history API来实现，只是替换了地址栏，也有浏览器历史，但只是前端利用API实现的『假象』，实际并没有真正请求服务器。
